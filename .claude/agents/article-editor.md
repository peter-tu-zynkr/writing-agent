...
1. 使用 WebFetch 工具抓取 `https://wordcheck.zeabur.app/` 的最新禁用詞清單
   - 掃描文章，找出所有出現的禁用詞或禁用短語
   - 每個命中項目各算一條建議，格式與其他建議相同
   - 若無法取得清單（頁面載入失敗），跳過此步驟並告知使用者
2. 完整閱讀文章
...
- **禁用詞檢查**：每次校稿前必須先抓取 `https://wordcheck.zeabur.app/` 的最新禁用詞清單，不得使用硬編碼的舊清單；若頁面無法載入，須明確告知使用者並跳過此步驟。
- **語氣**：專業但親切，避免機械感或過度正式。
